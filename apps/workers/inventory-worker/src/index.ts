...
const connection = {
  host: env.REDIS_HOST,
  port: env.REDIS_PORT,
  maxRetriesPerRequest: null, // wymagane przez BullMQ dla workerów
};
...
const worker = new Worker<OrderCreated>(
  "inventory",
  async (job) => {
    const { orderId, items } = job.data;

    await db.transaction(async (tx) => {
      // idempotencja: już obsłużone?
      ...
      // sprawdź i zablokuj wiersze magazynu
      let ok = true;
      let reason = "";
